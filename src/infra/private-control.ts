...
export function privateControl(options:{secret:string;origin:string;socketPath:string}) {
  ...
  return async(req:IncomingMessage,res:ServerResponse)=>{
    ...
    if(req.url==='/' && req.method==='GET') {
      res.setHeader('Content-Type','text/html; charset=utf-8');res.end(page(nonce));return;
    }
