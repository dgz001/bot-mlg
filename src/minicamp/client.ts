...
export function minicampCommand(text:string):boolean {
 return /^(?:![a-z]+(?:\s|$)|[123]$)/i.test(text)&&/^(?:!(?:novacopa|formato|entrar|resultado|confirmar|contestar|resolver|cancelar|copa|jogo|historico|minhascopas|campeoes|ranking|stats|ajuda|minicamp)(?:\s|$)|[123]$)/i.test(text);
}
