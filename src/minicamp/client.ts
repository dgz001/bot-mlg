...
export function minicampCommand(text:string):boolean {
 return /^(?:![a-zçí]+(?:\s|$)|[123]$)/i.test(text)&&/^(?:!(?:supabase|teste|titulo|título|sair|deletar|deletartitulo|anularcopa|clubes|comandos|confronto|forcarresultado|forcar|forçar|cancelarcopa|config|novacopa|formato|entrar|resultado|confirmar|contestar|resolver|cancelar|copa|jogo|historico|minhascopas|campeoes|ranking|stats|ajuda|minicamp)(?:\s|$)|[123]$)/i.test(text);
}
